...
class Check extends Component{
  ...
  onFormSubmit(e){
    ...
    this.fileUpload(this.state.file).then((response)=>{
      this.setState({nlp:response.data.nlp,
  ...
  fileUpload(file){
    const url = 'http://127.0.0.1:5000/api/video';
...
<h4>Sign Predicted from model : {this.state.sign}</h4>
<h4>Spell Checker output  : {this.state.nlp}</h4>
    </div>
      </div>): null}
      </div>
      </div>
